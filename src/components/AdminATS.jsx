...
function exportCandidateCSV(cand) {
  const headers = ["Name","Email","Phone","Source","Job Role","Department","Status","Experience (yrs)","Current Company","Current CTC","Expected CTC","Current Location","Preferred Location","Notice Period","Skills","Campaign","Education","Resume URL","Remarks"];
  const vals = [cand.name,cand.email,cand.phone,cand.source,cand.job_role,cand.department,cand.status,cand.experience,cand.current_company,cand.current_ctc,cand.expected_ctc,cand.current_location,cand.preferred_location,cand.notice_period,Array.isArray(cand.skills)?cand.skills.join("; "):(cand.skills||""),cand.campaign,cand.education,cand.resume_url,cand.remarks];
...
function exportCandidatePDF(cand) {
  const skills = Array.isArray(cand.skills) ? cand.skills.join(", ") : (cand.skills||"");
  const w = window.open("","_blank","width=800,height=900"); if (!w) return;
  w.document.write(`<!DOCTYPE html><html><head><title>${cand.name} — Candidate Profile</title>
  ...
  <tr><td>Full Name</td><td><b>${cand.name||"—"}</b></td></tr>
  <tr><td>Email</td><td>${cand.email||"—"}</td></tr>
  <tr><td>Phone</td><td>${cand.phone||"—"}</td></tr>
  <tr><td>Job Role</td><td>${cand.job_role||"—"}</td></tr>
  <tr><td>Department</td><td>${cand.department||"—"}</td></tr>
  <tr><td>Status</td><td>${cand.status||"—"}</td></tr>
  <tr><td>Source</td><td>${cand.source||"—"}</td></tr>
  <tr><td>Experience</td><td>${cand.experience?cand.experience+" years":"—"}</td></tr>
  <tr><td>Highest Education</td><td>${cand.education||"—"}</td></tr>
  <tr><td>Current Company</td><td>${cand.current_company||"—"}</td></tr>
  <tr><td>Current / Last CTC</td><td>${cand.current_ctc||"—"}</td></tr>
  <tr><td>Expected CTC</td><td>${cand.expected_ctc||"—"}</td></tr>
  <tr><td>Current Location</td><td>${cand.current_location||"—"}</td></tr>
  <tr><td>Preferred Location</td><td>${cand.preferred_location||"—"}</td></tr>
  <tr><td>Notice Period</td><td>${cand.notice_period||"—"}</td></tr>
  ${skills?`<tr><td>Skills</td><td>${skills}</td></tr>`:""}
  ${cand.remarks?`<tr><td>Remarks</td><td>${cand.remarks}</td></tr>`:""}
...
export default function AdminATS({ token, role = "admin", employees = [] }) {
  ...
  function exportAllCSV() {
    ...
    const headers = ["Name","Email","Phone","Source","Job Role","Department","Status","Experience (yrs)","Current Company","Current CTC","Expected CTC","Current Location","Notice Period","Skills"];
    const rows = data.map(c => [c.name,c.email,c.phone,c.source,c.job_role,c.department,c.status,c.experience,c.current_company,c.current_ctc,c.expected_ctc,c.current_location,c.notice_period,Array.isArray(c.skills)?c.skills.join("; "):(c.skills||"")]);
